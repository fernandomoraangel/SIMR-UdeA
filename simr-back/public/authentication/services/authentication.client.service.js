...
angular.module("authentication").factory("Authentication", [
  ...
  function ($http, $q, $interval, $window) {
    // Detectar URL dinámicamente - Usar rutas relativas para compartir cookies
    const API_URL = (function () {
      if (typeof window !== "undefined") {
        const hostname = window.location.hostname;

        // Si estamos en localhost (desarrollo), usar rutas relativas
        // para que Nginx redirija correctamente las peticiones
        if (hostname === "localhost") {
          return "/api/auth";
        }

        // En producción o dominios personalizados, usar URL completa
        const protocol = window.location.protocol;
        const port = window.location.port;
        return protocol + "//" + hostname + (port && port !== "80" && port !== "443" ? ":" + port : "") + "/api/auth";
      }
      // Fallback
      return "/api/auth";
    })();
