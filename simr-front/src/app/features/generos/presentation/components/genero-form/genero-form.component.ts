...
import { CreateGeneroRequest, UpdateGeneroRequest, GeneroRelacion } from '../../../domain/genero.interface';
  ...
  template: `
    <div class="form-container">
      <header class="form-header">
        ...
        <h1>{{ isEditMode ? 'Editar Género Musical' : 'Nuevo Género Musical' }}</h1>
      </header>
        ...
        <form [formGroup]="generoForm" (ngSubmit)="onSubmit()">
          <div class="form-section">
            <mat-form-field appearance="outline" class="campo">
              <mat-label>Nombre del género *</mat-label>
              <input matInput formControlName="nombre" placeholder="Ej: Bambuco venezolano" />
          ...
          <app-collapsible-section title="Géneros relacionados" icon="shuffle" [collapsed]="true">
            <div class="section-content">
              ...
              @if (generoRelacionadoItems.length > 0) {
          ...
          <app-collapsible-section title="Sistemas sonoros" icon="audiotrack" [collapsed]="true">
            <div class="section-content">
              <app-autocomplete-create
                apiEndpoint="sistemas"
                placeholder="Buscar sistema sonoro..."
                [selected]="sistemasSonorosItems"
                (selectedChange)="sistemasSonorosItems = $event"
              />
            </div>
          </app-collapsible-section>

          <app-collapsible-section title="Medios sonoros" icon="music_note" [collapsed]="true">
            <div class="section-content">
              <app-autocomplete-create
                apiEndpoint="medios"
                placeholder="Buscar medio sonoro..."
                [selected]="mediosSonorosItems"
                (selectedChange)="mediosSonorosItems = $event"
              />
            </div>
          </app-collapsible-section>
...
export class GeneroFormComponent implements OnInit {
  ...
  protected generoRelacionadoItems: GeneroRelacion[] = [];
  protected padresItems: GeneroRelacion[] = [];
  protected hijosItems: GeneroRelacion[] = [];
  protected idiomaItems: any[] = [];
  protected sistemasSonorosItems: any[] = [];
  protected mediosSonorosItems: any[] = [];
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe((params) => {
      ...
      if (id) {
        ...
        this.http.get(`${environment.apiUrl}/generos/${id}`).subscribe({
          next: (genero: any) => {
            this.loadGeneroData(genero);
          },
          error: (err) => console.error('[GeneroForm] error al cargar', err),
        });
      }
    });
  }
  ...
  private loadReferenceData() {
    ...
    this.http.get(`${apiUrl}/generos`).subscribe({
  ...
  private loadGeneroData(genero: any) {
    ...
    this.generoRelacionadoItems = genero.generoRelacionado || [];
    ...
    this.sistemasSonorosItems = (genero.sistemasSonoros || []).map((s: any) => {
      ...
      return { _id: s, nombre: '(cargando...)' };
    });
    this.mediosSonorosItems = (genero.mediosSonoros || []).map((m: any) => {
      if (typeof m === 'object') return { _id: m._id, nombre: m.nombre };
      return { _id: m, nombre: '(cargando...)' };
    });
    this.proyectosItems = (genero.proyectosAsociados || []).map((p: any) => {
  ...
  protected removeGeneroRelacionado(index: number) {
    this.generoRelacionadoItems = this.generoRelacionadoItems.filter((_, i) => i !== index);
  }
  ...
  protected onSubmit() {
    ...
    if (this.isEditMode && this.generoId) {
      this.generosService.update(this.generoId, payload).subscribe({
        next: () => this.router.navigate(['/generos', this.generoId]),
        error: (err) => {
          console.error('[GeneroForm] Error al actualizar:', err);
          this.store.setError(err?.error?.message || 'Error al actualizar el género');
        },
      });
    } else {
      this.generosService.create(payload).subscribe({
        ...
        error: (err) => {
          console.error('[GeneroForm] Error al crear:', err);
          this.store.setError(err?.error?.message || 'Error al crear el género');
        },
      });
    }
  }
