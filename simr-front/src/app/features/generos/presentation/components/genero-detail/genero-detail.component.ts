    ...
    <div class="detail-container">
      ...
      @if (store.isLoading()) {
        <div class="cargando">
          ...
          <span>Cargando detalles del género…</span>
        </div>
      }
      @if (store.hasError()) {
        <div class="alerta">
      ...
      @if (store.selectedGenero(); as g) {
        <mat-card class="ficha" appearance="outlined">
          ...
          @if (g.generoRelacionado && g.generoRelacionado.length > 0) {
            ...
            <app-collapsible-section title="Idiomas" icon="language">
              <div class="chips-wrapper">
                @for (i of g.idioma; track $index) {
                  <span class="detalle-chip">{{ i.nombre }}</span>
                }
              </div>
            </app-collapsible-section>
          }

          @if (g.sistemasSonoros && g.sistemasSonoros.length > 0) {
            <app-collapsible-section title="Sistemas sonoros" icon="audiotrack">
              <div class="chips-wrapper">
                @for (s of g.sistemasSonoros; track $index) {
                  <span class="detalle-chip">{{ s.nombre }}</span>
                }
              </div>
            </app-collapsible-section>
          }

          @if (g.mediosSonoros && g.mediosSonoros.length > 0) {
            <app-collapsible-section title="Medios sonoros" icon="music_note">
              <div class="chips-wrapper">
                @for (m of g.mediosSonoros; track $index) {
                  <span class="detalle-chip">{{ m.nombre }}</span>
                }
              </div>
            </app-collapsible-section>
          }

          @if (g.proyectosAsociados && g.proyectosAsociados.length > 0) {
            <app-collapsible-section title="Proyectos asociados" icon="assignment">
              <div class="kv-list">
                @for (p of g.proyectosAsociados; track $index) {
                  <div class="kv-item">
        ...
        @if (!store.isLoading()) {
          <div class="empty-state">
            ...
            <button mat-stroked-button routerLink="/generos">Volver al listado</button>
          </div>
        }
      }
    </div>
...
export class GeneroDetailComponent implements OnInit {
  ...
  confirmDelete(id: string) {
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Eliminar género musical', message: `¿Confirma eliminar "${name}"?`, confirmText: 'Eliminar', danger: true },
    });
