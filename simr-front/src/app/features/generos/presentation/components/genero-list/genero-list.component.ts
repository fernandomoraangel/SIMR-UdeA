...
import { GenerosStore } from '../../../state/generos.store';
...
const ALL_FIELDS = [
  ...
  { key: 'generoRelacionado', label: 'Gén. relacionados' },
  { key: 'padres', label: 'Padres' },
  { key: 'hijos', label: 'Hijos' },
  { key: 'descripcion', label: 'Descripción' },
  { key: 'idioma', label: 'Idiomas' },
  { key: 'sistemasSonoros', label: 'Sist. sonoros' },
  { key: 'mediosSonoros', label: 'Medios sonoros' },
  { key: 'proyectosAsociados', label: 'Proyectos' },
...
const PREFS_KEY_TABLE = 'generoTableFields';
const PREFS_KEY_CARD = 'generoCardFields';

const TABLE_COL_MAP: Record<string, TableColumn> = {
  nombre: { key: 'nombre', label: 'Nombre', sortable: true, truncateTo: 30 },
  generoRelacionado: { key: 'generoRelacionado', label: 'Gén. relacionados', type: 'number', sortable: true },
  padres: { key: 'padres', label: 'Padres', type: 'number', sortable: true },
  hijos: { key: 'hijos', label: 'Hijos', type: 'number', sortable: true },
  descripcion: { key: 'descripcion', label: 'Descripción', sortable: true, truncateTo: 40 },
  idioma: { key: 'idioma', label: 'Idiomas', type: 'number', sortable: true },
  sistemasSonoros: { key: 'sistemasSonoros', label: 'Sist. sonoros', type: 'number', sortable: true },
  mediosSonoros: { key: 'mediosSonoros', label: 'Medios sonoros', type: 'number', sortable: true },
  proyectosAsociados: { key: 'proyectosAsociados', label: 'Proyectos', type: 'number', sortable: true },
...
@Component({
  ...
  template: `
    <div class="generos-list-container">
      ...
      @if (store.hasGeneros() && !store.isLoading()) {
        <div class="toolbar">
          <p class="simr-codigo">Total: {{ filteredGeneros().length }} de {{ store.generosCount() }}</p>
        ...
        @if (effectiveView() === 'cards') {
          <div class="generos-grid">
            @for (g of filteredGeneros(); track g._id) {
              <mat-card class="genero-card" appearance="outlined" (click)="navigateToDetail(g._id)">
                <mat-card-header>
                  <mat-card-title>{{ g.nombre }}</mat-card-title>
                </mat-card-header>
                <mat-card-content>
                  <div class="meta">
                    @if (cardFieldVisible('generoRelacionado') && g.generoRelacionado?.length) {
                      <div class="meta-row">
                        <mat-icon>shuffle</mat-icon>
                        <span>{{ g.generoRelacionado.length }} relacionado(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('padres') && g.padres?.length) {
                      <div class="meta-row">
                        <mat-icon>arrow_upward</mat-icon>
                        <span>{{ g.padres.length }} padre(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('hijos') && g.hijos?.length) {
                      <div class="meta-row">
                        <mat-icon>arrow_downward</mat-icon>
                        <span>{{ g.hijos.length }} hijo(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('idioma') && g.idioma?.length) {
                      <div class="meta-row">
                        <mat-icon>language</mat-icon>
                        <span>{{ g.idioma.length }} idioma(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('sistemasSonoros') && g.sistemasSonoros?.length) {
                      <div class="meta-row">
                        <mat-icon>audiotrack</mat-icon>
                        <span>{{ g.sistemasSonoros.length }} sistema(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('mediosSonoros') && g.mediosSonoros?.length) {
                      <div class="meta-row">
                        <mat-icon>music_note</mat-icon>
                        <span>{{ g.mediosSonoros.length }} medio(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('proyectosAsociados') && g.proyectosAsociados?.length) {
                      <div class="meta-row">
                        <mat-icon>folder</mat-icon>
                        <span>{{ g.proyectosAsociados.length }} proyecto(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('anotacionCartograficoTemporal') && g.anotacionCartograficoTemporal?.length) {
                      <div class="meta-row">
                        <mat-icon>map</mat-icon>
                        <span>{{ g.anotacionCartograficoTemporal.length }} anotación(es)</span>
                      </div>
                    }
                    @if (cardFieldVisible('descriptorLibre') && g.descriptorLibre?.length) {
                      <div class="meta-row">
                        <mat-icon>local_offer</mat-icon>
                        <span>{{ g.descriptorLibre.length }} descriptor(es)</span>
                      </div>
                    }
                    @if (cardFieldVisible('vinculoRelacionado') && g.vinculoRelacionado?.length) {
                      <div class="meta-row">
                        <mat-icon>language</mat-icon>
                        <span>{{ g.vinculoRelacionado.length }} enlace(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('archivosAdjuntos') && g.archivosAdjuntos?.length) {
                      <div class="meta-row">
                        <mat-icon>attach_file</mat-icon>
                        <span>{{ g.archivosAdjuntos.length }} archivo(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('creador') && g.creador) {
                      <div class="meta-row">
                        <mat-icon>person</mat-icon>
                        <span>{{ getCreadorName(g.creador) }}</span>
                      </div>
                    }
                    @if (cardFieldVisible('creado') && g.creado) {
                      <div class="meta-row">
                        <mat-icon>calendar_today</mat-icon>
                        <span>{{ g.creado | date:'dd/MM/yyyy' }}</span>
                      </div>
                    }
                  </div>
                </mat-card-content>
                <mat-card-actions align="end">
                  <button mat-icon-button (click)="$event.stopPropagation(); navigateToEdit(g._id)" matTooltip="Editar">
                    <mat-icon>edit</mat-icon>
                  </button>
                  <button mat-icon-button color="warn" (click)="$event.stopPropagation(); confirmDelete(g._id)" matTooltip="Eliminar">
      ...
      @if (!store.hasGeneros() && !store.isLoading()) {
        <div class="vacio">
          ...
          <h3>No hay géneros musicales registrados</h3>
          <p>Aún no se ha catalogado ningún género musical en el archivo.</p>
          <a mat-raised-button color="primary" routerLink="create">
            <mat-icon>add</mat-icon>
            Agregar Género
          </a>
  ...
  `,
  styles: [`
    .generos-list-container { padding: 2rem; max-width: 1200px; margin: 0 auto; }
    ...
    .generos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 1.25rem; }
    .genero-card { border-radius: 14px !important; border-color: var(--mat-sys-outline) !important; transition: transform 0.2s ease, box-shadow 0.2s ease; cursor: pointer; }
    .genero-card:hover { transform: translateY(-3px); box-shadow: 0 8px 22px rgba(31, 42, 36, 0.14) !important; }
...
export class GeneroListComponent implements OnInit {
  protected readonly store = inject(GenerosStore);
  ...
  private readonly STORAGE_KEY = 'simr-view-generos';
  ...
  protected filteredGeneros = computed(() => {
    const generos = this.store.generos();
    const term = this.searchTerm().toLowerCase().trim();
    if (!term) return generos;
  ...
  protected effectiveView = computed<ViewMode>(() => {
    ...
    return this.filteredGeneros().length > this.AUTO_TABLE_THRESHOLD ? 'table' : 'cards';
  });
  ...
  navigateToEdit(id: string) {
    this.store.setInitialState();
    this.router.navigate(['/generos/edit', id]);
  }
  ...
  confirmDelete(id: string) {
    const genero = this.store.generos().find((g) => g._id === id);
    const name = genero?.nombre || 'este género';
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Eliminar género', message: `¿Confirma eliminar "${name}"?`, confirmText: 'Eliminar', danger: true },
    });
