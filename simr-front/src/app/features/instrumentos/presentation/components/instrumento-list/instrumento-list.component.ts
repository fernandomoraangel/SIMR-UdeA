...
const ALL_FIELDS = [
  ...
  { key: 'clasificacion', label: 'Clasificación HS' },
...
const PREFS_KEY_TABLE = 'instrumentoTableFields';
const PREFS_KEY_CARD = 'instrumentoCardFields';

const TABLE_COL_MAP: Record<string, TableColumn> = {
  nombre: { key: 'nombre', label: 'Nombre', sortable: true, truncateTo: 30 },
  clasificacion: { key: 'clasificacion', label: 'Clasificación HS', sortable: true },
...
@Component({
  ...
  template: `
    <div class="instrumentos-list-container">
      <header class="header">
        <div>
          <p class="simr-eyebrow">Términos · Instrumentos musicales</p>
      ...
      @if (store.hasInstrumentos() && !store.isLoading()) {
        <div class="toolbar">
          <p class="simr-codigo">Total: {{ filteredInstrumentos().length }} de {{ store.instrumentosCount() }}</p>
        ...
        @if (effectiveView() === 'cards') {
          <div class="instrumentos-grid">
            @for (inst of filteredInstrumentos(); track inst._id) {
              <mat-card class="inst-card" appearance="outlined" (click)="navigateToDetail(inst._id)">
                <mat-card-header>
                  <mat-card-title>{{ inst.nombre }}</mat-card-title>
                  @if (inst.clasificacion) {
                    <span class="hs-badge">{{ inst.clasificacion }}</span>
                  }
                </mat-card-header>
                <mat-card-content>
                  <div class="meta">
                    @if (cardFieldVisible('alias') && inst.alias?.length) {
                      <div class="meta-row">
                        <mat-icon>alt_route</mat-icon>
                        <span>
                          @for (a of inst.alias; track $index) {
                            <span class="badge">{{ a.nombre }}</span>
                          }
                        </span>
                      </div>
                    }
                    @if (cardFieldVisible('proyectosAsociados') && inst.proyectosAsociados?.length) {
                      <div class="meta-row">
                        <mat-icon>folder</mat-icon>
                        <span>{{ inst.proyectosAsociados.length }} proyecto(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('anotacionCartograficoTemporal') && inst.anotacionCartograficoTemporal?.length) {
                      <div class="meta-row">
                        <mat-icon>map</mat-icon>
                        <span>{{ inst.anotacionCartograficoTemporal.length }} anotación(es)</span>
                      </div>
                    }
                    @if (cardFieldVisible('descriptorLibre') && inst.descriptorLibre?.length) {
                      <div class="meta-row">
                        <mat-icon>local_offer</mat-icon>
                        <span>{{ inst.descriptorLibre.length }} descriptor(es)</span>
                      </div>
                    }
                    @if (cardFieldVisible('vinculoRelacionado') && inst.vinculoRelacionado?.length) {
                      <div class="meta-row">
                        <mat-icon>language</mat-icon>
                        <span>{{ inst.vinculoRelacionado.length }} enlace(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('archivosAdjuntos') && inst.archivosAdjuntos?.length) {
                      <div class="meta-row">
                        <mat-icon>attach_file</mat-icon>
                        <span>{{ inst.archivosAdjuntos.length }} archivo(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('creador') && inst.creador) {
                      <div class="meta-row">
                        <mat-icon>person</mat-icon>
                        <span>{{ getCreadorName(inst.creador) }}</span>
                      </div>
                    }
                    @if (cardFieldVisible('creado') && inst.creado) {
                      <div class="meta-row">
                        <mat-icon>calendar_today</mat-icon>
                        <span>{{ inst.creado | date:'dd/MM/yyyy' }}</span>
                      </div>
                    }
                  </div>
                </mat-card-content>
                <mat-card-actions align="end">
                  <button mat-icon-button (click)="$event.stopPropagation(); navigateToEdit(inst._id)" matTooltip="Editar">
                    <mat-icon>edit</mat-icon>
                  </button>
                  <button mat-icon-button color="warn" (click)="$event.stopPropagation(); confirmDelete(inst._id)" matTooltip="Eliminar">
      ...
      @if (!store.hasInstrumentos() && !store.isLoading()) {
        <div class="vacio">
          ...
          <p>Aún no se ha catalogado ningún instrumento musical en el archivo.</p>
          <a mat-raised-button color="primary" routerLink="create">
            <mat-icon>add</mat-icon>
            Agregar Instrumento
          </a>
  ...
  `,
  styles: [`
    .instrumentos-list-container { padding: 2rem; max-width: 1200px; margin: 0 auto; }
    ...
    .instrumentos-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 1.25rem; }
    .inst-card { border-radius: 14px !important; border-color: var(--mat-sys-outline) !important; transition: transform 0.2s ease, box-shadow 0.2s ease; cursor: pointer; }
    .inst-card:hover { transform: translateY(-3px); box-shadow: 0 8px 22px rgba(31, 42, 36, 0.14) !important; }
    mat-card-title { font-family: var(--simr-display); font-weight: 600; font-size: 1.15rem; color: var(--simr-tinta); }
    .hs-badge { background: var(--simr-musgo); color: white; border-radius: 4px; padding: 0.1rem 0.45rem; font-family: 'IBM Plex Mono', monospace; font-size: 0.75rem; margin-left: 0.5rem; }
...
export class InstrumentosListComponent implements OnInit {
  ...
  private readonly STORAGE_KEY = 'simr-view-instrumentos';
  ...
  protected filteredInstrumentos = computed(() => {
    const instrumentos = this.store.instrumentos();
    const term = this.searchTerm().toLowerCase().trim();
    if (!term) return instrumentos;
    return instrumentos.filter((m) =>
  ...
  protected effectiveView = computed<ViewMode>(() => {
    ...
    return this.filteredInstrumentos().length > this.AUTO_TABLE_THRESHOLD ? 'table' : 'cards';
  });
  ...
  confirmDelete(id: string) {
    const inst = this.store.instrumentos().find((m) => m._id === id);
