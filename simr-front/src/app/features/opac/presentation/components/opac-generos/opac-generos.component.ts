...
import { OpacGeneroResult } from '../../../opac.models';

@Component({
  selector: 'app-opac-generos',
  standalone: true,
  ...
  template: `
    <div class="opac-page">
      ...
      <div class="opac-search">
        <mat-icon class="search-icon">category</mat-icon>
        <input
          type="text"
          [(ngModel)]="query"
          (keyup.enter)="search()"
      ...
      @if (!loading() && !error() && results().length === 0 && searched()) {
        <div class="opac-empty">
          ...
          <p>No se encontraron géneros con "{{ lastQuery }}"</p>
        </div>
      }

      <div class="opac-results">
        @for (gen of results(); track gen._id) {
          <div class="opac-card">
            <div class="card-header">
              <h2 class="card-title">{{ gen.nombre }}</h2>
            </div>

            @if (gen.obras?.length) {
              <h3 class="section-title">Obras ({{ gen.obras.length }})</h3>
              @for (obra of gen.obras; track obra._id) {
                <div class="sub-card">
              ...
              <span class="no-data">No hay obras registradas con este género</span>
            }
          </div>
        }
      </div>
...
export class OpacGenerosComponent {
  ...
  error = signal('');
  results = signal<OpacGeneroResult[]>([]);
  ...
  search() {
    ...
    this.opacService.searchByGenero(q).subscribe({
      next: (res) => { this.results.set(res.results); this.loading.set(false); },
      error: (err) => {
        this.error.set(err.error?.message || 'Error al buscar géneros');
