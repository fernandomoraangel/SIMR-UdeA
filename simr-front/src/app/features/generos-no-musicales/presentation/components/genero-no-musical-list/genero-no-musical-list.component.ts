...
const ALL_FIELDS = [
  ...
  { key: 'generosRelacionados', label: 'Gén. relacionados' },
...
const PREFS_KEY_TABLE = 'generoNoMusicalTableFields';
const PREFS_KEY_CARD = 'generoNoMusicalCardFields';
...
const TABLE_COL_MAP: Record<string, TableColumn> = {
  ...
  generosRelacionados: { key: 'generosRelacionados', label: 'Gén. relacionados', type: 'number', sortable: true },
...
@Component({
  selector: 'app-genero-no-musical-list',
  standalone: true,
  imports: [
  ...
  template: `
    <div class="list-container">
      <header class="header">
        <div>
          <p class="simr-eyebrow">Términos · Géneros no musicales</p>
          <h1>Géneros no musicales</h1>
        </div>
          ...
          <div class="generos-grid">
            @for (g of filteredGeneros(); track g._id) {
              <mat-card class="genero-card" appearance="outlined" (click)="navigateToDetail(g._id)">
                ...
                <mat-card-content>
                  <div class="meta">
                    @if (cardFieldVisible('alias') && g.alias?.length) {
                      <div class="meta-row">
                    ...
                    @if (cardFieldVisible('generosRelacionados') && g.generosRelacionados?.length) {
                      <div class="meta-row">
                        <mat-icon>shuffle</mat-icon>
                        <span>{{ g.generosRelacionados.length }} relacionado(s)</span>
                      </div>
                    }
      ...
      @if (!store.hasGeneros() && !store.isLoading()) {
        <div class="vacio">
          ...
          <h3>No hay géneros no musicales registrados</h3>
          <p>Aún no se ha catalogado ningún género no musical en el archivo.</p>
          <a mat-raised-button color="primary" routerLink="create">
            <mat-icon>add</mat-icon>
            Agregar Género
          </a>
...
export class GeneroNoMusicalListComponent implements OnInit {
  ...
  private readonly STORAGE_KEY = 'simr-view-generosnomusicales';
