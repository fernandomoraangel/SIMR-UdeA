...
const PREFS_KEY_TABLE = 'medioTableFields';
const PREFS_KEY_CARD = 'medioCardFields';
...
@Component({
  ...
  template: `
    <div class="medios-list-container">
      <header class="header">
        <div>
          <p class="simr-eyebrow">Recursos · Medios sonoros</p>
          <h1>Medios sonoros</h1>
        </div>
      ...
      @if (store.hasMedios() && !store.isLoading()) {
        <div class="toolbar">
          <p class="simr-codigo">Total: {{ filteredMedios().length }} de {{ store.mediosCount() }}</p>
        ...
        @if (effectiveView() === 'cards') {
          <div class="medios-grid">
            @for (medio of filteredMedios(); track medio._id) {
              <mat-card class="medio-card" appearance="outlined" (click)="navigateToDetail(medio._id)">
                <mat-card-header>
                  <mat-card-title>{{ medio.nombre }}</mat-card-title>
                </mat-card-header>
                <mat-card-content>
                  <div class="meta">
                    @if (cardFieldVisible('alias') && medio.alias?.length) {
                      <div class="meta-row">
                        <mat-icon>alt_route</mat-icon>
                        <span>
                          @for (a of medio.alias; track $index) {
                            <span class="badge">{{ a.nombre }}</span>
                          }
                        </span>
                      </div>
                    }
                    @if (cardFieldVisible('instrumentos') && medio.instrumentos?.length) {
                      <div class="meta-row">
                        <mat-icon>music_note</mat-icon>
                        <span>{{ medio.instrumentos.length }} instrumento(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('proyectosAsociados') && medio.proyectosAsociados?.length) {
                      <div class="meta-row">
                        <mat-icon>folder</mat-icon>
                        <span>{{ medio.proyectosAsociados.length }} proyecto(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('anotacionCartograficoTemporal') && medio.anotacionCartograficoTemporal?.length) {
                      <div class="meta-row">
                        <mat-icon>map</mat-icon>
                        <span>{{ medio.anotacionCartograficoTemporal.length }} anotación(es)</span>
                      </div>
                    }
                    @if (cardFieldVisible('descriptorLibre') && medio.descriptorLibre?.length) {
                      <div class="meta-row">
                        <mat-icon>local_offer</mat-icon>
                        <span>{{ medio.descriptorLibre.length }} descriptor(es)</span>
                      </div>
                    }
                    @if (cardFieldVisible('vinculoRelacionado') && medio.vinculoRelacionado?.length) {
                      <div class="meta-row">
                        <mat-icon>language</mat-icon>
                        <span>{{ medio.vinculoRelacionado.length }} enlace(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('archivosAdjuntos') && medio.archivosAdjuntos?.length) {
                      <div class="meta-row">
                        <mat-icon>attach_file</mat-icon>
                        <span>{{ medio.archivosAdjuntos.length }} archivo(s)</span>
                      </div>
                    }
                    @if (cardFieldVisible('creador') && medio.creador) {
                      <div class="meta-row">
                        <mat-icon>person</mat-icon>
                        <span>{{ getCreadorName(medio.creador) }}</span>
                      </div>
                    }
                    @if (cardFieldVisible('creado') && medio.creado) {
                      <div class="meta-row">
                        <mat-icon>calendar_today</mat-icon>
                        <span>{{ medio.creado | date:'dd/MM/yyyy' }}</span>
                      </div>
                    }
                  </div>
                </mat-card-content>
                <mat-card-actions align="end">
                  <button mat-icon-button (click)="$event.stopPropagation(); navigateToEdit(medio._id)" matTooltip="Editar">
                    <mat-icon>edit</mat-icon>
                  </button>
                  <button mat-icon-button color="warn" (click)="$event.stopPropagation(); confirmDelete(medio._id)" matTooltip="Eliminar">
      ...
      @if (!store.hasMedios() && !store.isLoading()) {
        <div class="vacio">
          ...
          <h3>No hay medios sonoros registrados</h3>
          <p>Aún no se ha catalogado ningún medio sonoro en el archivo.</p>
          <a mat-raised-button color="primary" routerLink="create">
            <mat-icon>add</mat-icon>
            Agregar Medio
          </a>
  ...
  `,
  styles: [`
    .medios-list-container { padding: 2rem; max-width: 1200px; margin: 0 auto; }
    ...
    .medios-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 1.25rem; }
    .medio-card { border-radius: 14px !important; border-color: var(--mat-sys-outline) !important; transition: transform 0.2s ease, box-shadow 0.2s ease; cursor: pointer; }
    .medio-card:hover { transform: translateY(-3px); box-shadow: 0 8px 22px rgba(31, 42, 36, 0.14) !important; }
...
export class MediosListComponent implements OnInit {
  ...
  private readonly STORAGE_KEY = 'simr-view-medios';
  ...
  protected filteredMedios = computed(() => {
    const medios = this.store.medios();
    const term = this.searchTerm().toLowerCase().trim();
    if (!term) return medios;
  ...
  protected effectiveView = computed<ViewMode>(() => {
    ...
    return this.filteredMedios().length > this.AUTO_TABLE_THRESHOLD ? 'table' : 'cards';
  });
  ...
  navigateToDetail(id: string) {
    this.store.setInitialState();
    this.router.navigate(['/medios', id]);
  }

  confirmDelete(id: string) {
    const medio = this.store.medios().find((m) => m._id === id);
    const name = medio?.nombre || 'este medio';
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      data: { title: 'Eliminar medio', message: `¿Confirma eliminar "${name}"?`, confirmText: 'Eliminar', danger: true },
    });
    dialogRef.afterClosed().subscribe((ok) => {
      if (ok) this.store.deleteMedio(id);
    });
  }
}
