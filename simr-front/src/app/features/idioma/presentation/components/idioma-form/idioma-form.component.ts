...
export class IdiomaFormComponent implements OnInit {
  ...
  constructor() {
    ...
    // Effect para rellenar el formulario cuando el idioma es cargado
    effect(() => {
    ...
    // Effect para redirigir automáticamente si la operación fue exitosa
    // effect(() => {
    //   if (this.store.success() && !this.store.isLoading()) {
    //     console.log('[idioma-form] Operación exitosa:');
    //     this.goBack();
    //   }
    // });
  }
  ...
  ngOnInit() {
    this.store.setInitialState(); // Resetear el estado al iniciar
  ...
  onSubmit() {
    if (this.idiomaForm.valid) {
      ...
      if (this.isEditMode && this.idiomaId) {
        console.log('Updating idioma:', this.idiomaId, formValue);
        this.store.updateIdioma({ id: this.idiomaId, data: formValue });
      } else {
        console.log('Creating new idioma:', formValue);
